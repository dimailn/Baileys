...
class WAConnection extends _3_Connect_1.WAConnection {
    constructor() {
        ...
        this.emitParticipantsUpdate = (jid, participants, action) => {
            ...
            if (meta) {
                switch (action) {
                    case 'add':
                        participants.forEach(id => (meta.participants.push({ id, isAdmin: false, isSuperAdmin: false })));
                        break;
                    case 'remove':
                        meta.participants = meta.participants.filter(p => !participants.includes(Utils_1.whatsappID(p.id)));
                        break;
                    case 'promote':
                    case 'demote':
                        const isAdmin = action === 'promote';
                        meta.participants.forEach(p => {
                            if (participants.includes(Utils_1.whatsappID(p.id)))
                                p.isAdmin = isAdmin;
                        });
                        break;
                }
    ...
    applyingPresenceUpdate(update) {
        ...
        if (chat && jid.endsWith('@s.whatsapp.net')) { // if its a single chat
            ...
            else if (update.type === Constants_1.Presence.unavailable && presence.lastKnownPresence !== Constants_1.Presence.unavailable) {
